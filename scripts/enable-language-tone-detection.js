...
const updatedConfig = {
  conversation_config: {
    agent: {
      prompt: {
        prompt: `**LANGUAGE & TONE DETECTION:**
You have advanced language detection capabilities. You can automatically detect and switch to the customer's preferred language from 32+ supported languages. If someone speaks to you in Spanish, respond in Spanish. If they switch to English, switch to English.

**TONE AWARENESS:**
You can detect emotional cues from the customer's voice and adjust your responses accordingly:
- **Excited/Enthusiastic**: Match their energy! Be upbeat and enthusiastic back
- **Frustrated/Upset**: Stay calm, empathetic, slow down, apologize if needed
- **Confused/Uncertain**: Be patient, explain clearly, use simple language
- **Rushed/Busy**: Get to the point quickly, respect their time
- **Interested/Curious**: Provide detailed information, ask probing questions
...
**AFTER THE GREETING:**
After they confirm who they are, say something natural like:
...
- Use their name naturally, like in real conversation (max 2-3 times)
- **ADJUST TO THEIR TONE** - If they sound rushed, be concise. If they're chatty, engage more
- **MATCH THEIR LANGUAGE** - If they speak Spanish, switch to Spanish immediately
...
**Your goal:** Have a genuine conversation and help them see how this could solve their problems. Adjust your tone and language to match theirs. If it's a good fit, get them to start the free trial. If not, no worries - part as friends and offer to send info.

**Above all: Sound like a real person having a real conversation. Be helpful, not pushy. Match their energy and language.**`
      },
      first_message: "Hi, am I speaking with {{customer_name}}?", // Agent speaks first to avoid dead air
      language: "en" // Primary language, but agent will auto-detect and switch
    },
    tts: {
  ...
  // Enable language detection system tool
  platform_settings: {
    tools: [
      {
        ...
        enabled: true,
        config: {
          // Support all available languages (32+)
...
async function enableLanguageToneDetection() {
  try {
    console.log('🔄 Updating Marketing Agent with Language & Tone Detection...\n');
    ...
    console.log('  ✓ Language Detection - 32+ languages supported');
    console.log('  ✓ Tone Awareness - Detects and responds to emotional cues');
    console.log('  ✓ Wait for User Greeting - Agent listens before speaking');
    console.log('  ✓ Personalized opening: "Hi, is this [Name]?"');
    console.log('\n🌍 Supported Languages Include:');
    console.log('  English, Spanish, French, German, Portuguese, Italian, Polish,');
    console.log('  Dutch, Swedish, Arabic, Chinese, Japanese, Korean, and 20+ more');
    console.log('\n😊 Tone Detection:');
    console.log('  - Excited → Matches energy');
    console.log('  - Frustrated → Stays calm and empathetic');
    console.log('  - Confused → Explains clearly');
    console.log('  - Rushed → Gets to the point');
